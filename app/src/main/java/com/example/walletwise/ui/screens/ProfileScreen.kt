...
fun ProfileScreen(navController: NavHostController) {
    Scaffold(
        ...
        Column(
            ...
            ProfileOption(icon = R.drawable.ic_leaderboard, label = "Leaderboard")
            ProfileOption(icon = R.drawable.ic_favorites, label = "Retos Favoritos")

            Spacer(modifier = Modifier.height(24.dp))

            // Opción de premium
            ProfileOption(icon = R.drawable.ic_premium, label = "Pásate a Premium", isPremium = true)
        }
    }
}

@Composable
fun ProfileOption(icon: Int, label: String, isPremium: Boolean = false, onClick: (() -> Unit)? = null) {
    Row(
        modifier = Modifier
            .fillMaxWidth()
